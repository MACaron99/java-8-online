...
public class Main {
    ...
    private static void program() {
        ...
        System.out.println("Welcome to the program STRING. A simple a program with a little functionality, but can reverse the string using three methods");
    }
    ...
    private static void option(String position, BufferedReader reader) throws IOException {
        switch (position) {
            ...
            case "2" -> {
                if (Turner.exist()) System.out.println(Turner.reverse(Turner.src));
                else System.out.println("You didn't create a string");
            }
            case "3" -> {
                if (Turner.exist()) {
                    System.out.println("Please enter the substring");
                    System.out.println(Objects.requireNonNullElse(Turner.reverse(Turner.src, reader.readLine()), "No match found"));
                } else System.out.println("You didn't create a string");
            }
            case "4" -> {
                if (Turner.exist()) {
